	/// $$ compile options
// #define STUSB4500_DISABLE_PDO2
// #define STUSB4500_DISABLE_PDO3
...
#include "common.h"
#include "i2c_task.h"
#include "i2c_bpm_bq40z50.h"
#include "i2c_bpm_bq40z50_print.h"
...
void i2c_task_task(void *pvParameters)
{
	...
	if (i2c_usb_pd_stusb4500_read_device_id(&device_id))
		...
		/// Write PDO2 - 15V/5A - imposible current limitation, so change to 5V
	union stusb4500_pdo_t pdo2_ =
	{
#ifdef STUSB4500_DISABLE_PDO2
		.pdo.operational_current_10ma = 5000 / 10,  /// 5A
#else
		.pdo.operational_current_10ma = 1000 / 10,  /// 1A
#endif
		.pdo.voltage_50mv = 15000 / 50,             /// 15V
	...
	if (i2c_usb_pd_stusb4500_write_pdo_register(2, &pdo2_))
		PRINTF("Wrote PDO2 ok.\r\n");
	else
		PRINTF("Can't write PDO2.\r\n");

		/// Write PDO3 - 20V/5A - imposible current limitation, so change to 15V
	union stusb4500_pdo_t pdo3_ =
	{
#ifdef STUSB4500_DISABLE_PDO3
		.pdo.operational_current_10ma = 5000 / 10,  /// 5A
#else
		.pdo.operational_current_10ma = 1000 / 10,  /// 1A
#endif
	...
	if (i2c_usb_pd_stusb4500_write_pdo_register(3, &pdo3_))
		PRINTF("Wrote PDO3 ok.\r\n");
	else
		PRINTF("Can't write PDO3.\r\n");
